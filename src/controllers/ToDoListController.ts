...
export class ToDoListController {
  ...
  public async getToDoListsForUser(
    ...
    const { limit, offset } = await Joi.object({
    ...
    const [lists, total] = await this.toDoListService.getToDoListsForUser(
      req.user.id,
      offset,
      limit,
    );
  ...
  public async createToDoListForUser(
    ...
    const { title, items } = await Joi.object({
  ...
  public async updateToDoList(
    ...
    const { todolistId, title } = await Joi.object({
  ...
  public async removeToDoList(
    @Req() req: Request,
    @Res() res: Response,
  ): Promise<Response<unknown>> {
    const { todolistId } = await Joi.object({
      todolistId: Joi.string().uuid().required(),
    }).validateAsync({
      todolistId: req.params.todolistId,
    });
