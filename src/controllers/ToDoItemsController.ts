...
export class ToDoItemController {
  ...
  public async getToDoList(
    ...
    const { limit, offset, todolistId } = await Joi.object({
    ...
    const [items, total] = await this.toDoItemService.getTodoItemsForUser(
      req.user.id,
      todolistId,
      offset,
      limit,
  ...
  public async addTodoItems(
    @Req() req: Request,
    @Res() res: Response,
  ): Promise<Response<unknown>> {
    logger.info('todolistId', req.params.todolistId);
    const { todolistId, items } = await Joi.object({
      todolistId: Joi.string().uuid().required(),
      items: Joi.array().items(Joi.string().min(3).max(255)).min(1).required(),
    }).validateAsync({
      todolistId: req.params.todolistId,
      items: req.body,
    });
  ...
  @Patch('/items')
  public async updateTodoItemsForUser(
    @Req() req: Request,
    @Res() res: Response,
  ): Promise<Response<unknown>> {
    ...
    const _items = await this.toDoItemService.updateTodoItemsForUser(
      req.user.id,
      items,
    );
  ...
  @Delete('/items')
  public async removeTodoItemsForUser(
    @Req() req: Request,
    @Res() res: Response,
  ): Promise<Response<unknown>> {
    const { itemIds } = await Joi.object({
