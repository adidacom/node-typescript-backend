...
export class ToDoItemService {
  public async getTodoItemsForUser(
    userId: string,
    toDoListId: string,
    offset: number,
    limit: number,
  ): Promise<[ToDoItem[], number]> {
  ...
  public async updateTodoItemsForUser(
    ...
    const toDoItemIds = items
      .filter(
        (item) => item.description != undefined || item.completed != undefined,
      )
      .map((item) => item.id);
    ...
    const [_items, total] = await toDoItemRepo
  ...
  public async removeTodoItemsForUser(
    ...
    const [items, total] = await toDoItemRepo
      .createQueryBuilder('item')
      .leftJoin('item.toDoList', 'list')
      .whereInIds(toDoItemIds)
      .andWhere('list.userId = :userId', { userId })
