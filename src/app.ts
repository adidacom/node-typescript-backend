  ...
  public async startExpressServer(): Promise<Server> {
    ...
    if (server) {
      ...
      if (config.env === 'development') {
        ...
        const [users, total] = await this.userService.getAllUsers(0, 3, [
          'username',
          'apiKey',
        ]);
        if (total > 0) {
          logger.debug(
            'These are some API Keys that you may use for this demo:',
            users,
          );
        } else {
          logger.debug(
            "Hey, I couldn't find any users in the database. You'll have to create some to test the API.",
          );
        }
      }
    }
